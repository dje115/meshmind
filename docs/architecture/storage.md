...
| `documents_view` | Normalized documents (entity cards, runbooks) |
| `facts_view` | Fact aggregates (counts, sums, time-window metrics) |
...
```json
{
  "event_id": "evt-001",
  "type": 1,
  "ts": { "unix_ms": 1709000000000 },
  "node_id": { "value": "node-a1b2" },
  "sensitivity": 1,
  "prev_hash": "",
  "event_hash": "sha256:abc123...",
  "payload": { "case_created": { "case_id": "case-001", "title": "DNS Timeout", ... } }
}
```
...
Objects are raw bytes. Typical layout for a JSON artifact:

- **Path**: `objects/sha256/ab/cd/abcdef1234567890...`
- **Content**: JSON or binary (e.g. artifact body, model weights)
